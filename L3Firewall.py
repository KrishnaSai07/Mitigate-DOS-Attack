...
class Firewall (EventMixin):
	...
	def _handle_PacketIn(self, event):
		...
		if(match.dl_type == packet.IP_TYPE):
		  ...
		  # implement the port security logic in here
		  if(match.dl_src in self.portTable.keys()):
		  	if(match.nw_src in self.portTable.values()):
		  		log.debug("Expected route flows")
		  	else:
		  		if((match.dl_src,None) not in self.disbaled_MAC_pair):
		  			self.disbaled_MAC_pair.append((match.dl_src,None))
